...
def test_drop_invalid_rows_removes_rows_with_missing_values():
...
def test_drop_invalid_rows_raises_keyerror_for_unknown_column():
    """Test que verifica que el método drop_invalid_rows lanza un KeyError cuando
    se intenta eliminar filas inválidas de una columna que no existe en el DataFrame.
    
    Escenario esperado:
    - Crear un DataFrame usando make_sample_df()
    - Intentar llamar a drop_invalid_rows con una columna que no existe (ej: "does_not_exist")
...
def test_trim_strings_raises_typeerror_for_non_string_column():
    """Test que verifica que el método trim_strings lanza un TypeError cuando
    se intenta aplicar trim a una columna que no es de tipo string.
    
    Escenario esperado:
    - Crear un DataFrame usando make_sample_df()
    - Intentar llamar a trim_strings con una columna numérica (ej: "age")
...
def test_remove_outliers_iqr_raises_keyerror_for_missing_column():
    """Test que verifica que el método remove_outliers_iqr lanza un KeyError cuando
    se intenta eliminar outliers de una columna que no existe en el DataFrame.
    
    Escenario esperado:
    - Crear un DataFrame usando make_sample_df()
    - Intentar llamar a remove_outliers_iqr con una columna que no existe (ej: "salary")
    - Verificar que se lanza un KeyError
    """


def test_remove_outliers_iqr_raises_typeerror_for_non_numeric_column():
    """Test que verifica que el método remove_outliers_iqr lanza un TypeError cuando
    se intenta eliminar outliers de una columna que no es de tipo numérico.
    
    Escenario esperado:
    - Crear un DataFrame usando make_sample_df()
    - Intentar llamar a remove_outliers_iqr con una columna de texto (ej: "city")
    - Verificar que se lanza un TypeError
    """
